...
/** Single-row pill trigger shared by the dropdowns and the location
 *  picker's button — icon inline, fixed height matching a plain search input
 *  beside it, so every control in the filter bar sits on one baseline. */
...
export function FilterDropdown({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="border-ink-100 absolute z-20 mt-2 min-w-[200px] rounded-2xl border bg-white p-1.5 shadow-[0_16px_50px_-12px_rgba(0,86,94,0.18)]">
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
